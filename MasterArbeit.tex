...
%
%dilepton, semileptonic, nonallhad in samples nochmal überprüfen, ob das jetzt so stimmt...      
\input{header.tex}
\begin{document}
\input{titlepage.tex}
\tableofcontents
...
\begin{figure}
 \centering
  ...
  \quad
  \begin{subfigure}[t]{0.45\textwidth}
   \includegraphics[width=1\textwidth]{figures/t_hadronic.pdf}
...
\begin{figure}
 \centering
  ...
  \end{subfigure}
  \begin{subfigure}[t]{0.4\textwidth}
   \includegraphics[width=0.95\textwidth]{figures/ttbarb.pdf}
  ...
  \end{subfigure}
 \begin{subfigure}[t]{0.4\textwidth}
   \includegraphics[width=0.95\textwidth]{figures/ttbarc.pdf}
  ...
  \end{subfigure}
  \begin{subfigure}[t]{0.4\textwidth}
   \includegraphics[width=0.95\textwidth]{figures/ttbard.pdf}
...
\label{binomialerror}
\end{align}
The total efficiency of reconstructing a hadronic tau matched to its truth particle and passing the basic selection over taking all truth taus fullfilling the basic selections (truth matching for tau) is as low as $\SI{0.635\pm 0.061}{\percent}$ in case of the top-quark pair events compared to $\SI{29.39\pm 0.26}{\percent}$ for the Higgs background events. The matching is realized with a geometric matching of the reconstructed and truth particle within a cone of $\Delta R=\sqrt{\Delta\phi^2+\Delta\eta^2}<0.05$. This fact reflects that no taus are expected in the former sample and some tau leptons are expected from direct Higgs boson decays. As a conclusion the tau reconstruction efficiency of the $t\bar{t}$ represents a fake tau rate.\par 
%	
\begin{sidewaystable}[htbp]
		\centering
                \footnotesize
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttbar/Divided_Tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_Tau.pdf}
...
\begin{figure}
  \centering
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_Jet.pdf}
...
%
%Jet
The qualitative trend is for both the same showing a peak fake rate at $p_{T,\text{jet}}^{\text{truth}}$ of $\SI{50}{\giga\electronvolt}$. Before the efficiency goes down there exist a plateau in the range of the truth jet transverse momentum between $\SI{100}{\giga\electronvolt}$ and $\SI{200}{\giga\electronvolt}$. The main difference is that the fake rate for the $t\bar{t}H$ sample is greater than that of the $t\bar{t}$ sample. Differences in the production processes of jets can be responsible for that behaviour, because jets in the top-pair simulation are mostly induced by gluons (cf. fig. \ref{ttbarproduction}) while jets in the Higgs simulation are mostly induced by associated quark generation including the special case of top quarks (cf. fig. \ref{ttHproduction}).\par
%b-jet
The fake rate for b-jets is defined in the same way as for jets above with the difference of selecting only jets of b-flavor. The efficiency values of the two background samples are not in agreement within the uncertainties, altough they are in the same range of $\SI{0.1616\pm 0.0022}{\percent}$ and $\SI{0.1496\pm 0.0033}{\percent}$ corresponding to $t\bar{t}$ and $t\bar{t}H$. The efficiency depending on the truth transverse momentum of the b-jet is shown in figure \ref{DividedBjet:bg}.
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttbar/Divided_Bjet.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_Bjet.pdf}
...
\end{figure}
%
Comparing the large difference between both samples for jet fakes with the small difference of b-jet fakes shows the dominance of b-jets in the top-quark pair related events, whereas the Higgs related events are more dominated by light jets. This can be seen in the efficiency sub-selection of taking only light jets into account and excluding the b-jets explicitly. This is done in figure \ref{Divided_NoBjet:bg}, which shows the constant high efficiency for the Higgs related background in the range up to $\SI{120}{\giga\electronvolt}$ for the truth b-jets' transverse momentum (see fig. \ref{Divided_NoBjet:bg:ttH}). Sources of b-jets are the subsequent decay of top-quark pairs mainly in b-quarks or in case of Higgs related background from direct Higgs decays into $b\bar{b}$. \par%to do: b kommen halt mal schnell von ttbar zerfall, aber woher kommen b bei H? gibt es ja keine wirkliche Quelle, oder? hab ich oben ja schon gesagt, dass tt nicht so vorkommt, jetzt ist nur die frage, eigentlich hat ja auch H->bb einen anteil... aber ist wohl nicht so groß im vergleich zu den haufen ttbars in ttbar sample?
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttbar/Divided_NoBjet.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_NoBjet.pdf}
...
\begin{figure}
  \centering
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_prompt1prong.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_prompt3prong.pdf}
...
\end{figure}
%
The efficiency derived from the number of taus passing the basic selection and are matched to a truth tau within $\Delta R=0.05$ originating from a Higgs boson over the total number of truth taus passing the basic selection and originate from a Higgs boson is $\SI{45.20\pm 0.47}{\percent}$. The efficiency plot is shown in figure \ref{DividedFromH:bg:all}. Splitting the taus' origin from $H^0$ into 1-prong and 3-prong case gives the efficiency of $\SI{49.25\pm 0.55}{\percent}$ and $\SI{34.08\pm 0.87}{\percent}$ respectively (see figure \ref{DividedFromH:bg:1prong} and \ref{DividedFromH:bg:3prong}). Compared to the efficiency resulting from the product of the different object reconstruction efficiencies the tau efficiency coming from Higgs decays are 15 percencentage points and 5 percentage point higher for the 1-prong and 3-prong case respectively.\par    
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromH.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromH1prong.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromH3prong.pdf}
...
\begin{align}
S_{T,\text{lep}}&=\cancel{E}_{T}+\sum_i\p_{T,i}^{\text{e}}+\sum_i\p_{T,i}^{\mu}+\sum_i\p_{T,i}^{\tau}\\
H_{T,\text{had}}&=\sum_i\p_{T,i}^{\text{jet}}\\
S_{T}&=\cancel{E}_{T}+\sum_i\p_{T,i}^{\text{e}}+\sum_i\p_{T,i}^{\mu}+\sum_i\p_{T,i}^{\tau}+\sum_i\p_{T,i}^{\text{jet}}\\
H_{T}&=\sum_i\p_{T,i}^{\text{e}}+\sum_i\p_{T,i}^{\mu}+\sum_i\p_{T,i}^{\text{jet}}\,\text{,}
\label{STgedöns}
\end{align}
where $p_{T,i}^\text{X}$ is the transverse momentum of single electrons $X=e$, muons $X=\mu$, taus $X=\tau$ and jets $X=\text{jet}$ and $\cancel{E}_{T}$ denotes the missing energy, which includes in the standard model considerations the neutrinos. With the definitions of eq. (\ref{STgedöns}) different sensitivities are covered. $S_T$ is sensitive to all considered objects with the sub-category of $S_{T,\text{lep}}$, which only takes the leptonic objects into account, while $H_{T}$ is only sensitive to electrons and muons as well as jets. $H_{T,\text{had}}$ only includes jets.\par
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_promptHT.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_promptHThad.pdf}
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromHHT.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromHHThad.pdf}
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_promptST.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_promptSTlep.pdf}
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromHST.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromHSTlep.pdf}
...
\end{figure}
%
The sum of all considered objects of $S_T$ (see \ref{Divided:prompt:ST}) or the sub-group $S_{T,\text{lep}}$ (see \ref{Divided:prompt:STlep}) show slight efficiency variations over the hole momentum range for taus originated from weak gauge bosons. Efficiencies for taus originated from Higgs bosons are depicted in figure \ref{Divided:fromH:STgedöns}. Noticable is the small peak at $\SI{850}{\giga\electronvolt}$ and $\SI{450}{\giga\electronvolt}$ of the otherwise constant efficiency around the total efficiency value of $\SI{45.20\pm 0.47}{\percent}$. Due to the large uncertainty and the fact that also other plots include this feature its origin seems to be of a systematic error.\newline  
%njets
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_promptnjets.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromHnjets.pdf}
...
Decay characteristics will influence the seperation of different objects involved in the event selection. Therefore the minimum and maximum seperation $\Delta R$ is studied between taus and leptons, taus and jets as well as taus and b-jets. This is done seperately for the taus' decay induced from weak gauge bosons and Higgs bosons.\par
Figure \ref{dR:bg:taulepton} shows the minimum and maximum seperation between taus and leptons (electrons and muons) for both origins, namely $W^\pm$, $Z^0$ and $H^0$.\newline
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_pr_mindR_taulepton.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and leptons.}
                \label{dR:bg:prompt:taulepton:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_maxdR_pr_taulepton.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and leptons.}
                \label{dR:bg:prompt:taulepton:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromH_mindR_taulepton.pdf}
                \subcaption{Minimum seperation between taus originating from the Higgs events and leptons.}
                \label{dR:bg:fromH:taulepton:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_maxdR_fromH_taulepton.pdf}
                \subcaption{Maximum seperation between taus originating from the Higgs events and leptons.}
                \label{dR:bg:fromH:taulepton:max}
                \end{subfigure}
\caption[Efficiency of taus originating from $W^\pm$, $Z^0$ and Higgs bosons for the seperation between taus and leptons.]{Efficiency of taus originating from $W^\pm$, $Z^0$ and Higgs bosons for the minumum and maximum seperation between taus and leptons (electrons and muons).}
\label{dR:bg:taulepton}
\end{figure}
%
The minimum and maximum seperation bewtween taus and jets is presented in figure \ref{dR:bg:taujets}. For both cases of the taus' origin they seem to be either well separated explained by the peak at $\Delta R=3.0$ for the maximum seperation. Or there is no separation shown by the abundant entries for $\Delta R<0.5$, which become the main fake rate contributors for the category jets.\newline
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_pr_mindR_taujets.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and jets.}
                \label{dR:bg:prompt:taujets:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_maxdR_pr_taujets.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and jets.}
                \label{dR:bg:prompt:taujets:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_fromH_mindR_taujets.pdf}
                \subcaption{Minimum seperation between taus originating from the Higgs events and jets.}
                \label{dR:bg:fromH:taujets:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_maxdR_fromH_taujets.pdf}
                \subcaption{Maximum seperation between taus originating from the Higgs events and jets.}
                \label{dR:bg:fromH:taujets:max}
                \end{subfigure}
\caption[Efficiency of taus originating from $W^\pm$, $Z^0$ and Higgs bosons for the seperation between taus and jets.]{Efficiency of taus originating from $W^\pm$, $Z^0$ and Higgs bosons for the minumum and maximum seperation between taus and jets.}
\label{dR:bg:taujets}
\end{figure}
%
The maximum separation between taus and b-jets has the difference that the separation peak is slightly shifted to $\Delta R=2.8$ compared to the jet separation (see figure \ref{dR:bg:prompt:taubjets:max} and \ref{dR:bg:fromH:taubjets:max}). This is valid for taus coming from Higgs. The decay of the weak gauge bosons as tau lepton source remains unchanged compared to the jet separation. But the minimum separation between taus and b-jets show large changes compared to all jet flavors (see figure \ref{dR:bg:prompt:taubjets:min} and \ref{dR:bg:fromH:taubjets:max}). The minimum separation is extended up to $\Delta R=2.0$ and is significantly broadend compared to the separation of taus and jets. While many entries remain in the low separation region of $\Delta R<0.5$ the efficiency drops down to nearly zero.
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_mindR_pr_taubjets.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and bjets.}
                \label{dR:bg:prompt:taubjets:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_maxdR_pr_taubjets.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and bjets.}
                \label{dR:bg:prompt:taubjets:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_mindR_fromH_taubjets.pdf}
                \subcaption{Minimum seperation between taus originating from the Higgs events and bjets.}
                \label{dR:bg:fromH:taubjets:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/ttH/Divided_maxdR_fromH_taubjets.pdf}
                \subcaption{Maximum seperation between taus originating from the Higgs events and bjets.}
                \label{dR:bg:fromH:taubjets:max}
                \end{subfigure}
\caption[Efficiency of taus originating from $W^\pm$, $Z^0$ and Higgs bosons for the seperation between taus and bjets.]{Efficiency of taus originating from $W^\pm$, $Z^0$ and Higgs bosons for the minumum and maximum seperation between taus and bjets.}
\label{dR:bg:taubjets}
\end{figure}
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Signal efficiency}\label{signaleff}
The efficiencies for the leptoquark signal sampels with their two mass points are summarized in table \ref{LQEffTable} following the example of the background efficiencies in section \ref{bgeff}. The uncertainty is calculated derived from the binomial error of the efficiency after eq. (\ref{binomialerror}). The efficiency are the same as for the background samples with the difference that the category of a tau originating from a Higgs boson replaced by the categry of tau leptons originating from leptoquarks.\par
%	
	\begin{sidewaystable}[htbp]
		\centering
                \footnotesize
...
%prompt
The weak gauge bosons remain as tau sources with an efficiency of $\SI{32.4\pm 4.4}{\percent}$ for the LQ$_{500\textbf{GeV}}$ sample. This is comparable with the efficiency in the same sub-category for the Higgs background sample (cf. table \ref{ttbarttHEffTable}). But the higher mass point LQ sample shows a smaller efficiency of $\SI{20.8\pm 3.3}{\percent}$ in that sub-selection. This means that the behaviour regarding tau sources from $W^\pm$ and $Z^0$ is comparable in both signal and $t\bar{t}H$ background samples. The efficiency depending on the truth taus' transverse momentum is shown in figure \ref{Divided:signal:prompt:LQ75} and \ref{Divided:signal:prompt:LQ76} for the low and the high LQ mass point respectively. A further sub-selection differentiates between the tau decay into one and three charged particles, which can be seen in figure \ref{Divided:signal:prompt1prong:LQ75} and \ref{Divided:signal:prompt3prong:LQ75} for the low mass point. The $p_{T,\text{vis}}^{\tau,\text{truth}}$ range reaches $\SI{300}{\giga\electronvolt}$ and $\SI{250}{\giga\electronvolt}$ and shows an increasing efficiency with increasing transverse momentum. This effect is known from the Higgs background events. The overall efficiency for the 1-prong case is $\SI{32.9\pm 5.3}{\percent}$ and for the 3-prong case is $\SI{34.5\pm 8.8}{\percent}$. For the high mass point the 1-prong and 3-prong decay is shown in figure \ref{Divided:signal:prompt1prong:LQ76} and \ref{Divided:signal:prompt3prong:LQ76} and has efficiencies of $\SI{23.6\pm 4.1}{\percent}$ and $\SI{12.5\pm 5.8}{\percent}$ respectively.\par 
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_prompt.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_prompt1prong.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_prompt3prong.pdf}
...
\label{Divided:signal:prompt:LQ75}
\end{figure}
%
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_prompt.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_prompt1prong.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_prompt3prong.pdf}
...
\begin{figure}
  \centering
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQ1prong.pdf}
                ...
                \label{DividedfromLQ:bg:1prongLQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQ3prong.pdf}
...
\label{Divided:signal:fromLQ:LQ75}
\end{figure}
%
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQ.pdf}
                \subcaption{Efficiency of taus originating LQs of $\SI{50.7\pm 1.5}{\percent}$ for the high mass LQ events.}
                \label{DividedfromLQ:bg:all}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQ1prong.pdf}
                ...
                \label{DividedfromLQ:bg:1prong}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQ3prong.pdf}
...
\label{Divided:signal:fromLQ:LQ76}
\end{figure}
%
%HT,ST Gedöns prompt | LQ
The correlation between the efficiency and the variables $H_T$, $H_{T,\text{had}}$, $S_T$, $S_{T,\text{lep}}$ defined in eq. (\ref{STgedöns}) and the number of jets is also available for the leptoquark related events. \newline
%HT und HThad
Figure \ref{Dividedprompt:HTgedöns} shows the efficiency depending on the variable $H_T$ and $H_{T,\text{had}}$ for the sub-selections, where taus originate from weak gauge.
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQHT.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $H_{T}$ low mass LQ sample.}
                \label{Divided:prompt:HT:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQHThad.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $H_{T,\text{had}}$ low mass LQ sample.}
                \label{Divided:prompt:HThad:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQHT.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $H_{T}$ high mass LQ sample.}
                \label{Divided:prompt:HT:LQ76}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQHThad.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $H_{T,\text{had}}$ high mass LQ sample.}
                \label{Divided:prompt:HThad:LQ76}
                \end{subfigure}
\caption[Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons for the LQ singal events.]{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons depending on $H_{T}$ (a), (c) and $H_{T,\text{had}}$ (b), (d) for the LQ singal events.}
\label{Dividedprompt:HTgedöns}
\end{figure}
%
The correlation between $H_{T}$ ($H_{T,\text{had}}$) and the efficiency in case of the sub-category, where tau leptons originate from LQs, are shown in figure \ref{DividedFromLQ:HTgedöns}. The $H_T$ ($H_{T,\text{had}}$) spectrum covers a larger range for the $\SI{1}{\tera\electronvolt}$ leptoquarks sample due to its higher mass. The $H_T$ distribution extends up to $\SI{3}{\tera\electronvolt}$ (see fig. \ref{Divided:fromLQ:HT:LQ76}) compared to figure \ref{Divided:fromLQ:HT:LQ75}, which extends only to $\SI{2.5}{\tera\electronvolt}$. The $H_{T,\text{had}}$ distribution extends up to $\SI{2.5}{\tera\electronvolt}$ (see fig. \ref{Divided:fromLQ:HThad:LQ76}) compared to figure \ref{Divided:fromLQ:HThad:LQ75}, which extends to $\SI{1.2}{\tera\electronvolt}$. The efficiency trend over the hole spectrum is constant with the most fluctuation in case of the $H_{T}$ spectrum for the high mass point, besides the $H_{T,\text{had}}$ spectrum of the low mass point. It shows lower efficiencies at the edge of the sum of the jets' transverse momentum from $SI{200}{\giga\electronvolt}$ to $SI{300}{\giga\electronvolt}$ and from $SI{900}{\giga\electronvolt}$ to $SI{1000}{\giga\electronvolt}$.\newline
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                ...
                \label{Divided:fromLQ:HT:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQHThad.pdf}
                \subcaption{Efficiency of taus originating from leptoquarks of depending on $H_{T,\text{had}}$ low mass LQ sample.}
                \label{Divided:fromLQ:HThad:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQHT.pdf}
                \subcaption{Efficiency of taus originating from leptoquarks of depending on $H_{T}$ high mass LQ sample.}
                \label{Divided:fromLQ:HT:LQ76}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQHThad.pdf}
                \subcaption{Efficiency of taus originating from leptoquarks of depending on $H_{T,\text{had}}$ high mass LQ sample.}
                \label{Divided:fromLQ:HThad:LQ76}
                \end{subfigure}
\caption[Efficiency of taus originating from leptoquarks for the LQ singal events.]{Efficiency of taus originating from leptoquarks depending on $H_{T}$ (a), (c) and $H_{T,\text{had}}$ (b), (d) for the LQ singal events.}
\label{DividedfromLQ:HTgedöns}
\end{figure}
%
%ST und STlep
Taking into account not only jets, electrons and muons, but also taus and missing energy for example one can study the correlation bewtween the efficiency and $S_T$ ($S_{T,\text{lep}}$). This is done in figure \ref{Dividedprompt:STgedöns} for taus originating from weak gauge bosons. Efficiencies are found to be in range of high momenta. For the $\SI{1}{\tera\electronvolt}$ sample the spectrum is even more shifted to higher $p_T$ due to the high mass point. For the $S_T$ spectrum the efficiency starts at $\SI{500}{\giga\electronvolt}$ (see fig. \ref{Divided:prompt:ST:LQ76}) and the $S_{T,\text{lep}}$ spectrum starts at $\SI{1}{\tera\electronvolt}$ (see fig. \ref{Divided:prompt:STlep:LQ76}). The sample with the low LQ mass point is different from the other LQ sample. It has a local efficiency minimum in the $\S_T$ distribution at $\SI{1.3}{\tera\electronvolt}$ (see fig. \ref{Divided:prompt:ST:LQ75}) and a local minimum in the $S_{T,\text{lep}}$ distribution at $SI{0.5}{\tera\electronvolt}$. \newline
%
\begin{figure}
  \centering    
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQST.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $S_{T}$ low mass LQ sample.}
                \label{Divided:prompt:ST:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQSTlep.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $S_{T,\text{lep}}$ low mass LQ sample.}
                \label{Divided:prompt:STlep:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQST.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $S_{T}$ high mass LQ sample.}
                \label{Divided:prompt:ST:LQ76}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQSTlep.pdf}
                \subcaption{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons of depending on $S_{T,\text{lep}}$ high mass LQ sample.}
                \label{Divided:prompt:STlep:LQ76}
                \end{subfigure}
\caption[Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons for the LQ singal events.]{Efficiency of taus originating from $W^\pm$ and $Z^0$ bosons depending on $S_{T}$ (a), (c) and $S_{T,\text{lep}}$ (b), (d) for the LQ singal events.}
\label{Dividedprompt:STgedöns}
\end{figure}
%
The sub-selection with taus originating directly from LQs show in this consideration of $S_T$ and $S_{T,\text{lep}}$ show no such minima in case of the sample with the low mass point. Their efficiency plots can be seen in figure \ref{Divided:fromLQ:ST:LQ75} and \ref{Divided:fromLQ:STlep:LQ75}. The $S_T$ and $S_{T,\text{lep}}$ distribution for the high mass point is shown in figure \ref{Divided:fromLQ:ST:LQ76} and \ref{Divided:fromLQ:STlep:LQ76} respectively. \newline 
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                ...
                \label{Divided:fromLQ:ST:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromLQSTlep.pdf}
                \subcaption{Efficiency of taus originating from leptoquarks of depending on $S_{T,\text{lep}}$ low mass LQ sample.}
                \label{Divided:fromLQ:STlep:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQST.pdf}
                \subcaption{Efficiency of taus originating from leptoquarks of depending on $S_{T}$ high mass LQ sample.}
                \label{Divided:fromLQ:ST:LQ76}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromLQSTlep.pdf}
                \subcaption{Efficiency of taus originating from leptoquarks of depending on $S_{T,\text{lep}}$ high mass LQ sample.}
                \label{Divided:fromLQ:STlep:LQ76}
                \end{subfigure}
\caption[Efficiency of taus originating from leptoquarks for the LQ singal events.]{Efficiency of taus originating from leptoquarks depending on $S_{T}$ (a), (c) and $S_{T,\text{lep}}$ (b), (d) for the LQ singal events.}
\label{DividedfromLQ:STgedöns}
\end{figure}
%
%njets
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_promptnjets.pdf}
                ...
                \label{Divided:prompt:njets:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_promptnjets.pdf}
                ...
                \label{Divided:prompt:njets:LQ76}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromHnjets.pdf}
                ...
                \label{Divided:fromH:njets:LQ75}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromHnjets.pdf}
...
%dR taulepton
The minimum and maximum separation between taus and leptons (electrons and muons) is shown in figure \ref{dR:signal:prompt:taulepton} for both signal samples. It is the efficiency for taus originating from $W^\pm$ and $Z^0$ bosons.   
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_pr_mindR_taulepton.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and leptons for the low mass point LQ sample.}
                \label{dR:signal:prompt:taulepton:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_maxdR_pr_taulepton.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and leptons for the low mass point LQ sample.}
                \label{dR:signal:prompt:taulepton:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_pr_mindR_taulepton.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and leptons for the high mass point LQ sample.}
                \label{dR:signal:fromH:taulepton:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_maxdR_pr_taulepton.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and leptons for the high mass point LQ sample.}
                \label{dR:signal:fromH:taulepton:max}
                \end{subfigure}
...
\label{dR:signal:prompt:taulepton}
\end{figure}
%
The minimum and maximum separation for the efficiency selection of taus originating from leptoquarks is depicted in figure \ref{dR:signal:fromH:taulepton}.\newline
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromH_mindR_taulepton.pdf}
                \subcaption{Minimum seperation between taus originating from Higgs events and leptons for the low mass point LQ sample.}
                \label{dR:signal:fromH:taulepton:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_maxdR_fromH_taulepton.pdf}
                \subcaption{Maximum seperation between taus originating from Higgs events and leptons for the low mass point LQ sample.}
                \label{dR:signal:fromH:taulepton:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromH_mindR_taulepton.pdf}
                \subcaption{Minimum seperation between taus originating from Higgs events and leptons for the high mass point LQ sample.}
                \label{dR:signal:fromH:taulepton:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_maxdR_fromH_taulepton.pdf}
                \subcaption{Maximum seperation between taus originating from Higgs events and leptons for the high mass point LQ sample.}
                \label{dR:signal:fromH:taulepton:max}
                \end{subfigure}
...
\label{dR:signal:fromH:taulepton}
\end{figure}
%
Correspondigly to the background samples the minimum and maximum separation between taus and jets is shown in figure \ref{dR:signal:prompt:taujet} for taus originating from $W^\pm$ and $Z^0$ and in figure \ref{dR:signal:fromH:taujet} for taus originating directly from LQs.\newline
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_pr_mindR_taujet.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and jets for the low mass point LQ sample.}
                \label{dR:signal:prompt:taujet:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_maxdR_pr_taujet.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and jets for the low mass point LQ sample.}
                \label{dR:signal:prompt:taujet:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_pr_mindR_taujet.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taujet:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_maxdR_pr_taujet.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taujet:max}
                \end{subfigure}
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromH_mindR_taujet.pdf}
                \subcaption{Minimum seperation between taus originating from Higgs events and jets for the low mass point LQ sample.}
                \label{dR:signal:fromH:taujet:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_maxdR_fromH_taujet.pdf}
                \subcaption{Maximum seperation between taus originating from Higgs events and jets for the low mass point LQ sample.}
                \label{dR:signal:fromH:taujet:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromH_mindR_taujet.pdf}
                \subcaption{Minimum seperation between taus originating from Higgs events and jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taujet:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_maxdR_fromH_taujet.pdf}
                \subcaption{Maximum seperation between taus originating from Higgs events and jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taujet:max}
                \end{subfigure}
...
\label{dR:signal:fromH:taujet}
\end{figure}
%
The special case of the minimum and maximum separation between taus and b-flavor jets is shown in figure \ref{dR:signal:fromH:taubjet} for the sub-category taus originating from $W^\pm$ and $Z^0$ bosons.
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_pr_mindR_taubjet.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and b-jets for the low mass point LQ sample.}
                \label{dR:signal:prompt:taubjet:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_maxdR_pr_taubjet.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and b-jets for the low mass point LQ sample.}
                \label{dR:signal:prompt:taubjet:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_pr_mindR_taubjet.pdf}
                \subcaption{Minimum seperation between taus originating from $W^\pm$, $Z^0$ events and b-jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taubjet:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_maxdR_pr_taubjet.pdf}
                \subcaption{Maximum seperation between taus originating from $W^\pm$, $Z^0$ events and b-jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taubjet:max}
                \end{subfigure}
...
\label{dR:signal:prompt:taubjet}
\end{figure}
%
The separation between taus and b-jets in case of the sub-selection, where taus originate from LQs are shown in figure \ref{dR:signal:fromH:taubjet}.
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_fromH_mindR_taubjet.pdf}
                \subcaption{Minimum seperation between taus originating from Higgs events and b-jets for the low mass point LQ sample.}
                \label{dR:signal:fromH:taubjet:min}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ75/Divided_maxdR_fromH_taubjet.pdf}
                \subcaption{Maximum seperation between taus originating from Higgs events and b-jets for the low mass point LQ sample.}
                \label{dR:signal:fromH:taubjet:max}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_fromH_mindR_taubjet.pdf}
                \subcaption{Minimum seperation between taus originating from Higgs events and b-jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taubjet:min}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/LQ76/Divided_maxdR_fromH_taubjet.pdf}
                \subcaption{Maximum seperation between taus originating from Higgs events and b-jets for the high mass point LQ sample.}
                \label{dR:signal:fromH:taubjet:max}
                \end{subfigure}
...
\label{dR:signal:fromH:taubjet}
\end{figure}
%
\section{Kinematics}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In section \ref{signalANDbgEff} signal and background efficiencies helps to understand especially the role of the tau lepton in the signal and background samples for the relevant $t\tau^{-}+\bar{t}\tau^{+}$ channel. The next step into the direction of signal studies is to compare the event yield for signal and background samples in the different channels of the event selection (cf. section \ref{eventselection}). This is done by studying kinematic variables for the tau lepton like the transverse momentum, angular distribution, the variables defined in eq. \ref{STgedöns} (cf. section \ref{bgeff}) and the separation between taus and objects like electrons, jets and b-jets. The comparison is done with the background and signal MC samples together with data of the data-taking years 2015 until 2018. This reveals, if the signature of the choosen event selection can be extracted and detected within the known standard model processes. The data in this context gives the possibility of reference how well data is resembled by the MC simulation, although this work is not a full LQ search. No systematic variations are applied on the Monte Carlo simulations, which would be the normal procedure for a full search. Furthermore $\SI{1}{\percent}$ of the 2017 data are missing. This corresponds to a luminosity loss of \SI{0.44}{\per\femto\barn} (cf. table \ref{datalumi}).\par
%tau pT
...
\end{figure}
%
The selection of at least two b-jets and at least one tau shows (see figure \ref{taupt:2b1tau}) that up to transverse momenta of $\SI{80}{\giga\electronvolt}$ the LQ signal is hidden from top-quark pair and Higgs contributions. But from that threshold on the momentum distribution of the low mass signal sample exceeds the background. For the high mass point sample this happens at $\SI{200}{\giga\electronvolt}$. The MC background samples agree well with data from $\SI{50}{\giga\electronvolt}$ onwards, besides one outlier. Before that value data exceeds the used background. The discrepancy can be explained such that  more contributions should be taken into account like vector bosons and additional jets, di-boson events or single-top contributions. Tightening the selection to at least two taus (see figure \ref{taupt:2b2tau}) will reveal even more signal compared to relatively small background. The event yield for data shows that only few events in this selection is expected. The disatvantage in this selection is the few events in general, which results in low statistics. The looser selection of taking only at least one b-jet and at least one tau (see figure \ref{taupt:1b1tau}) shifts the threshold for the signal up to $\SI{100}{\giga\electronvolt}$  and $\SI{230}{\giga\electronvolt}$ for the low and high mass point LQ sample respectively, but allows at the same time higher statistics. This effect is clearly visible in the selection of at least one b-jet and at least two tau leptons shown in figure \ref{taupt:1b2tau}. The discrepancy between data and MC in the lower $p_T$ range also exists here, as it depends not directly on the choosen selection.\par
%tauphi
The distribution of the azimuthal angle of the tau leptons for different event selection is shown in figure \ref{tauphi}. It gives the same picture as for the transverse momentum distribution inluding the lack of MC events. For the selection of at least two b-jets and at least one tau the signal samples are hidden by the background over the hole $\phi$ range (see figure \ref{tauphi:2b1tau} and \ref{tauphi:1b1tau}). Only the at least two tau selection reveals the LQ signal. Better statistics can be achieved by using the selection with at least one b-jet and at least two taus (see figure \ref{tauphi:1b2tau}).\par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_phi_2j2b_1tau.pdf}
                \subcaption{$\phi$ distribution of the tau leptons, when selecting at least two b-jets and at least one additional tau.}
                \label{tauphi:2b1tau}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_phi_2j2b_2tau.pdf}
                \subcaption{$\phi$ distribution of the tau leptons, when selecting at least two b-jets and at least two additional tau.}
                \label{tauphi:2b2tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_phi_2j1b_1tau.pdf}
                \subcaption{$\phi$ distribution of the tau leptons, when selecting at least one b-jets and at least one additional tau.}
                \label{tauphi:1b1tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_phi_2j1b_2tau.pdf}
...
%
%tau eta
The pseudorapidity distribution shows the same trend as the $p_T$ and $\phi$ distribution described above and is shown in figure \ref{taueta}.\par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_eta_2j2b_1tau.pdf}
                \subcaption{$\eta$ distribution of the tau leptons, when selecting at least two b-jets and at least one additional tau.}
                \label{taueta:2b1tau}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_eta_2j2b_2tau.pdf}
                \subcaption{$\eta$ distribution of the tau leptons, when selecting at least two b-jets and at least two additional tau.}
                \label{taueta:2b2tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_eta_2j1b_1tau.pdf}
                \subcaption{$\eta$ distribution of the tau leptons, when selecting at least one b-jets and at least one additional tau.}
                \label{taueta:1b1tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/tau_eta_2j1b_2tau.pdf}
...
%
%MET
Following the definition of eq. \ref{STgedöns} (cf. section \ref{bgeff}) the distribution of the missing energy for different selections is shown in figure \ref{MET}. The missing energy is not a optimal discriminant variable, because the signal is not well separated over the hole $\cancel{E}_T$ range in the selection of at least one b-jet and at least one tau (see \ref{MET:1b1tau}). The signal is in fact better separated in the selection of at least two b-jets and at least one tau as depicted in figure \ref{MET:2b1tau}. Also the at least two tau condition does not improve the separation (see figure \ref{MET:2b2tau} and \ref{MET:1b2tau}). Altough the studied $t\tau^{-}+\bar{t}\tau^{+}$ channel has sources of neutrinos responsible for missing transverse energy through the decay of tau leptons or leptonic top-quark decays, this is not enough to discriminate it from background neutrino contributions.\par   
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/MET_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/MET_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/MET_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/MET_2j1b_2tau.pdf}
...
%
%HT
The distribution of $H_T$ is shown in figure \ref{HT}. Because this distribution is the sum of the transverse momenta of electrons, muons and jets and therefore includes this is good discriminating variable for particles with high masses like the leptoquarks in the signal sample. Figure \ref{HT:2b1tau} shows that the background decreases from $\SI{600}{\giga\electron}$ on and reveals at least the $\SI{500}{\giga\electronvolt}$ mass LQ signal. The best extraction results from at least two tau selection shown in figure \ref{HT:2b2tau} and \ref{HT:1b2tau}.\par    
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HT_2j2b_1tau.pdf}
                \subcaption{$H_T$ distribution of the tau leptons, when selecting at least two b-jets and at least one additional tau.}
                \label{HT:2b1tau}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HT_2j2b_2tau.pdf}
                \subcaption{$H_T$ distribution of the tau leptons, when selecting at least two b-jets and at least two additional tau.}
                \label{HT:2b2tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HT_2j1b_1tau.pdf}
                \subcaption{$H_T$ distribution of the tau leptons, when selecting at least one b-jets and at least one additional tau.}
                \label{HT:1b1tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HT_2j1b_2tau.pdf}
...
%
%HThad
The variable $H_{T,\text{had}}$ in principle shows the same trend as the $H_T$ distribution for a discriminating variable. Nevertheless the effect of extracting the signal is not as good as for $H_T$. The reason is that less objects are included in the variable. $H_{T,\text{had}}$ only contains the sum of the jets' momenta. The plots are shown in figure \ref{HThad}. \par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HThad_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HThad_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HThad_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/HThad_2j1b_2tau.pdf}
...
%
%ST
The most objects are coverd by $S_T$ variable. Its distribution is shown in figure \ref{ST}. It is possible to differentiate between signal and background in the selection of at least on b-jet and at least one tau (see figure \ref{ST:1b1tau}) as well as in the selection of at least two b-jets and at least one tau (see figure \ref{ST:2b1tau}). Taking into account at least two tau leptons means an improvement of the signal selection (see figure \ref{ST:1b2tau} and \ref{2b2tau}). The high mass point sample can be distinguished the best in this selection. Due to ist high LQ mass the $S_T$ spectrum reaches high values up to $\SI{2.5}{\tera\electronvolt}$. \par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/ST_2j2b_1tau.pdf}
                \subcaption{$S_T$ distribution of the tau leptons, when selecting at least two b-jets and at least one additional tau.}
                \label{ST:2b1tau}
                \end{subfigure}
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/ST_2j2b_2tau.pdf}
                \subcaption{$S_T$ distribution of the tau leptons, when selecting at least two b-jets and at least two additional tau.}
                \label{ST:2b2tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/ST_2j1b_1tau.pdf}
                \subcaption{$S_T$ distribution of the tau leptons, when selecting at least one b-jets and at least one additional tau.}
                \label{ST:1b1tau}
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/ST_2j1b_2tau.pdf}
...
%
%STlep
Excluding the jets in the definition of $S_T$ results in the possibly discriminating variable $S_{T,\text{lep}}$ (see figure \ref{STlep}), which is also capable to show a difference in signal and background at $\SI{450}{\giga\electronvolt}$ (see figure \ref{STlep:2b1tau} and \ref{STlep:1b1tau}). The at least two tau selection gives similar results to that of $S_T$ (see figure \ref{STlep:2b2tau} and \ref{STlep:21b2tau}). \par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/STlep_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/STlep_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/STlep_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/STlep_2j1b_2tau.pdf}
...
%
%dR
Minimum and maximum separation between taus and leptons, taus and jets as well as taus and b-jets within the event selection are partly convenient for a discrimination between signal and background.\newline
%min dR <tau,lepton>
The minimum separation between taus and leptons are shown in figure \ref{mindR:taulepton} for the different selections. The best discrimination can be reached for the selection of at least two tau leptons preferring the case with at least 1-bjet due to enough statistics (see figure \ref{mindR:taulepton:2b2tau} and \ref{mindR:taulepton:1b2tau}). Within the selection of only one tau the signal is exceeded by background events (see figure \ref{mindR:taulepton:2b1tau} and \ref{mindR:taulepton:1b2tau}). \par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taulepton_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taulepton_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taulepton_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taulepton_2j1b_2tau.pdf}
...
%
%max dR taulepton
The maximum separation between taus and leptons (electrons and muons) are shown in figure \ref{maxdR:taulepton}. The best discrimination between signal and background can be achieved with the selection of at least two taus. Altough this is not the best option compared to $S_T$ variable for example.\par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taulepton_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taulepton_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taulepton_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taulepton_2j1b_2tau.pdf}
...
%
%min dR <tau,jet>
The minimum and maximum separation between taus and jets as shown in figure \ref{mindR:taujet} and \ref{maxdR:taujet} as discrimination option is also not the best option compared to $S_T$ or tau related distributions. The sinal events exceed the background only in the selection with at least two taus with the disatvantage of lower statistics.\par
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taujet_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taujet_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taujet_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taujet_2j1b_2tau.pdf}
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taujet_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taujet_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taujet_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taujet_2j1b_2tau.pdf}
...
%
%min dR <tau,bjet>
Following the example of the minium and maximum separation between taus and jets the separation between taus and b-jets gives similar results shown in figure \ref{mindR:taubjet} and \ref{maxdR:taubjet}. Taking this separating variable as discriminant is not the best option.
%
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taubjet_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taubjet_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taubjet_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/mindR_taubjet_2j1b_2tau.pdf}
...
\begin{figure}
  \centering
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taubjet_2j2b_1tau.pdf}
                ...
                %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taubjet_2j2b_2tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taubjet_2j1b_1tau.pdf}
                ...
                \end{subfigure}
                 %
                \begin{subfigure}[t]{0.45\textwidth}
                \includegraphics[width=0.95\textwidth]{figures/plots/kinLQ75/maxdR_taubjet_2j1b_2tau.pdf}
...
\end{figure}
%
%resumee dass, die  1b2tau selection eine ganz gute wahl ist eig!
%auch wenn LQ76 im 1tau nicht so gut abschneidet, so gewinnt es trotzdem in der 2tau selection wegen des langen spectrums folgend aus der 1tev masse
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%quantitative ausssage über wie die signifikanz bei der extraction des signal is
\appendix
\chapter{Appendix}
\section{Comparison bewteen the top-quark background samples (di-lepton versus non-allhadronic)}\label{dilORnonallhad}
%Einfach eine Tabelle mit event yield für ttbar dil und ttbar nonallhad in einem, um zu zeigen, dass eigentlich nonallhad benötigt wird, aber das so auch geht: einen satz entsprechend dann dazu in used samples
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\listoffigures
