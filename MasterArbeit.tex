...
\end{itemize}
Condition (iv) makes sure that the proton instability is avoided. Condition (v) only allows inter-generational interactions and large tree-level flavor changing neutral currents and flavor universalites. Condition (vi) supresses contributions to chirally meson decays like $\pi\rightarrow e\nu$. The LQ model, fulfilling (i)-(vi), is the so called minimal-Buchm\"{u}ller-R\"{u}ckl-Wyler effective model (mBRW) with coupling constant $\lambda$.\newline
Fourteen different leptoquark types are known and are listed in table \ref{LQstable}. 
%experimental (masses erklärung? versteht man das? ah, phd lmu gibt aufschluss) + table of lq von kuze br ratio etc
\par
%generic da wird auf hewett verwiese
%dann noch überblick, was jetzt sonst in den anderen papern stünde, das das auf alle Fälle abgeklärt ist
%hier auch dann generic models erwähnen S8 SUSYandLQ, dann ist der Anschluss auf der SUSY Seite besser: LQ->models->constraints->exp Suche aber freier->generic models->LQ like SUSY decays
%generic -> (v) ist dann halt nicht so streng, bei der generischen Suche
%die relevanten websites sind:
