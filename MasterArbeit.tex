...
\end{align}
with $|F|=0$ or $|F|=2$.
\item[(v)] Each LQ couples to only one single quark-lepton generation, i.e. there are three LQ families.
\item[(vi)] LQs has pure chiral couplings to the SM fermions.
\end{itemize}
Condition (iv) makes sure that the proton instability is avoided. Condition (v) only allows inter-generational interactions and large tree-level flavor changing neutral currents and flavor universalities. Condition (vi) supresses contributions to chiral meson decays like $\pi\rightarrow e\nu$. The LQ model, fulfilling conditions (i)-(vi), is the so called minimal-Buchm\"{u}ller-R\"{u}ckl-Wyler effective model (mBRW) with coupling constant $\lambda$. \cite{Kuze}\newline
...
\begin{figure}
  \centering
                ...
                \begin{subfigure}[t]{0.9\textwidth}
                \qquad\qquad
                \includegraphics[width=0.4\textwidth]{figures/gfc.pdf}
...
\begin{figure}[htbp]                                 
 \begin{center}                                       
  \includegraphics[width=0.4\linewidth]{figures/yukawa.pdf} 
   \caption[Feynman graph of a leptoquark decay gouverned by Yukawa coupling.]{Feynman graphs of a leptoquark decay into a lepton $l$ and a quark $q$ gouverned by Yukawa coupling $\lambda_{lq}=\sqrt{\beta}\lambda$. \cite{LQATLAS}}
  \label{YukawaLQ}                                     
 \end{center}
\end{figure}
%
...
Final states of high energy particle collision experiments, like the {\LHC}, is an extremely challenging endeavor. Typically hundreds of particles of different species are involved and their energy and momentum spectra a widely distributed over differnt orders of magnitudes. Understanding these complex processes are crucial to make physical predictions and search for new physics. Therefore Monte Carlo simulations (MC) are one important cornerstone to handle these scenarios. \cite{bigMC}\par
The MC chain for the {\ATLAS} detector has three main steps. This includes the generation of MC events with event generators, the simulation of the detector and the propagation of the particles with the associated interactions and the digitization of energy deposited in the detector's sensitive areas. The last step involves the simulation of the resulting signals in the readout system for comparison with real data. The detailed implemented simulation of the detector additionally considers realistic detector conditions and misalignments as well as the detector response to be as realistic as possible. This fact enhances the digital copy to furthermore test the effectiveness of proposed search strategies for specific signatures. \cite{ATLASSim}
%aus Verenas MA: ATLAS detector and physics performance: Technical design report, 2 1999. CERN-LHCC-99-015
%aus Verenas MA: G Cowan Statistical data analysis. Oxford science publications 1998
%comparing aspect, generators based on computational algorithm reling on pseudo random numbers, 

%-->relevant matrix element are to labourious to compute beyond the first few orders of perturbation theory even there are non-perturbative problems in quantum chromodynamics (confinement) -->using mc techniques bigMC to overcome or circumvent these problems

%achieve reliable pysical results the sim has to be corrected to data to reproduce data ePerf
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
